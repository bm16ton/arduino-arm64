...
void Adafruit_SPITFT::writePixels(uint16_t *colors, uint32_t len, bool block,
  ...
  if (connection == TFT_HARD_SPI) {
    ...
    if (bigEndian) {
      ...
      hwspi._spi->transferOut(colors, len*2);
    }
    else {
      ...
      do {
        ...
        hwspi._spi->transferOut(bigEndianData, pixelCount*2);
        len -= pixelCount;
      } while (len>0);
    }
    return;
...
void Adafruit_SPITFT::drawRGBBitmap(int16_t x, int16_t y, uint16_t *pcolors,
                                    int16_t w, int16_t h, bool bigEndian) {
  ...
  while (h--) {              // For each (clipped) scanline...
    writePixels(pcolors, w, bigEndian); // Push one (clipped) row
...
void Adafruit_SPITFT::SPI_WRITE16(uint16_t w) {
  if (connection == TFT_HARD_SPI) {
    ...
    hwspi._spi->transferOut(&outBuf, 2);
#else
...
void Adafruit_SPITFT::SPI_WRITE32(uint32_t l) {
  if (connection == TFT_HARD_SPI) {
    ...
    hwspi._spi->transferOut(&outBuf, 4);
#else
